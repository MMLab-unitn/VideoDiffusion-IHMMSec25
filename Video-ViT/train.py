...
def check_accuracy(loader, extractor, model, settings):
    model.eval()
    ...
    with torch.no_grad():
        with tqdm(loader, unit='batch', mininterval=0.5) as tbatch:
            ...
            for (data, label, _) in tbatch:
                ...
                features = extractor.get_features(data).to(device)
                ...
                if settings.invert_labels:
                    zeroaccuracy, oneaccuracy = oneaccuracy, zeroaccuracy
...
def train(loader, extractor, val_dataloader, model, settings):
    best_accuracy = 0
    lr_decay_counter = 0
    for epoch in range(0, settings.num_epochs):
        ...
        accuracy = check_accuracy(val_dataloader, extractor, model, settings)
...
if __name__ == "__main__":
    ...
    train(train_dataloader, extractor, val_dataloader, model, settings)